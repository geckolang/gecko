...
impl TypeCheckContext {
  ...
  // TODO: Why not move helper functions here?
}
...
impl TypeCheck for ast::ArrayIndexing {
  fn infer_type(&self, cache: &cache::Cache) -> Option<ast::Type> {
    let target_array_variable = &*cache
      .declarations
      .get(&self.target_key.unwrap())
      .unwrap()
      .as_ref()
...
impl TypeCheck for ast::IfStmt {
  fn type_check(&self, type_context: &mut TypeCheckContext, cache: &mut cache::Cache) {
    if !unify_with_primitive(self.condition.infer_type(cache), ast::PrimitiveType::Bool) {
      type_context
        .diagnostics
...
impl TypeCheck for ast::LetStmt {
  fn type_check(&self, type_context: &mut TypeCheckContext, cache: &mut cache::Cache) {
    ...
    if !unify_option(Some(&self.ty), value_type.as_ref(), cache) {
      type_context
        .diagnostics
...
fn unify(type_a: &ast::Type, type_b: &ast::Type, cache: &cache::Cache) -> bool {
  let unboxed_type_a = unbox_type(type_a, cache);
  let unboxed_type_b = unbox_type(type_b, cache);
...
fn unify_option(
  ...
  match (type_a, type_b) {
    (Some(type_a), Some(type_b)) => unify(&type_a, &type_b, cache),
    _ => type_a == type_b,
  }
}

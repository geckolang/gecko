...
pub struct NameResolutionPass {
  package_buffer: Option<node::Package>,
}

impl pass::Pass for NameResolutionPass {
  ...
  fn visit_stub(&mut self, stub: &mut node::Stub) -> pass::PassResult {
    match stub {
      node::Stub::Callable { name, value } => {
        ...
        let possible_target = package_symbol_table.get(name).unwrap();

        value = &mut Some(*possible_target);
      }
    };
